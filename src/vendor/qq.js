...
const getArtistCover = (artistmid) => {
    ...
    return "http://y.gtimg.cn/music/photo_new/T001R500x500M000" + artistmid + ".jpg"
}
...
export class QQ {
    ...
    static nextRadioTrack(channel, track) {
        return new Promise((resolve, reject) => {
            ...
            //是否命中缓存
            if (channel == QQ.RADIO_CACHE.channel && index < length - 1) {
        ...
        return new Promise((resolve, reject) => {
            ...
            getJson(url, reqBody).then((json) => {
                ...
                songList.forEach((item) => {
                    ...
                    const track = new Track(item.mid, QQ.CODE, item.title, artist, album, duration, cover)
